<?php 
    ...
    class Reservation extends Database{
        ...
        // === Update hour status from hold to free === // 
        public function updateReservationStatusToFree($reservatioDateDay, $reservation_date) {
            $sql = "UPDATE full_schedule SET hour_availability = :hour_status WHERE day_id = :day_id AND hour_value = :hour_value";
            $preparedSQL = $this->Dbh->prepare($sql);
            $preparedSQL->bindValue(':hour_status', 'free', PDO::PARAM_STR);
            $preparedSQL->bindValue(':day_id', $reservatioDateDay, PDO::PARAM_INT);
            $preparedSQL->bindValue(':hour_value', $reservation_date, PDO::PARAM_INT);
        ...
        public function deleteReservation($reservationID,$reservation_date,$reservatioDateDay){
            // delete the reservation from the database and update the hour status from hold to free
            ...
            if($preparedSQL->execute()){
                $updateReservationStatus = $this->updateReservationStatusToFree($reservatioDateDay, $reservation_date);
                if($updateReservationStatus){
                    return true; 
                }
            }else{
        ...
        {
            $sql = "SELECT * FROM reservations WHERE reservation_customer_reference = :reservation_costumer_reference";
            $preparedSQL = $this->Dbh->prepare($sql);
            $preparedSQL->bindValue(':reservation_costumer_reference', $userToken, PDO::PARAM_STR);
            if($preparedSQL->execute()) {
                return $preparedSQL->fetchAll(PDO::FETCH_OBJ);
            }else{
